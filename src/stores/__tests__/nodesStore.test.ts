...
describe('nodesStore', () => {
  ...
  describe('addNode', () => {
    ...
    it('sets selectedNodeId to the newly added node id', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(-23.55, -46.63, 'Test Node')
      store.addNode(node)
  ...
  describe('removeNode', () => {
    it('removes a node by id and leaves the list empty', () => {
      const store = useNodesStore()
    ...
    it('sets selectedNodeId to null when the selected node is removed', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(-23.55, -46.63, 'Node A')
      store.addNode(node)
      expect(store.selectedNodeId).toBe(node.id)
    ...
    it('does not affect other nodes when removing a specific one', () => {
      const store = useNodesStore()
  ...
  describe('updateNode', () => {
    it('updates the name of an existing node', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(-23.55, -46.63, 'Original Name')
      store.addNode(node)

      store.updateNode(node.id, { name: 'Updated Name' })
    ...
    it('does nothing when the node id does not exist', () => {
      ...
      // Update with a non-existent id — should not throw and list is unchanged
      store.updateNode('non-existent-id', { name: 'Ghost' })
  ...
  describe('applyDevicePreset', () => {
    it('applies the heltec_v3 preset: txPowerDbm=22, txPowerW=0.158', () => {
      const store = useNodesStore()
      ...
      store.applyDevicePreset(node.id, 'heltec_v3')
    ...
    it('applies the heltec_v3 preset frequency correctly', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(0, 0, 'Node')
      store.addNode(node)
    ...
    it('sets devicePresetId on the node', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(0, 0, 'Node')
      store.addNode(node)
    ...
    it('applies rxSensitivityDbm from the preset', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(0, 0, 'Node')
      store.addNode(node)
    ...
    it('does nothing when node id does not exist', () => {
      const store = useNodesStore()
      // Should not throw
      expect(() => store.applyDevicePreset('non-existent', 'heltec_v3')).not.toThrow()
    })

    it('does nothing when preset id does not exist', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(0, 0, 'Node')
      store.addNode(node)
      const originalPower = node.txPowerDbm

      store.applyDevicePreset(node.id, 'does_not_exist')
  ...
  describe('applyChannelPreset', () => {
    it('applies SHORT_TURBO preset: rxSensitivityDbm = -108', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(0, 0, 'Node')
      store.addNode(node)

      store.applyChannelPreset(node.id, 'SHORT_TURBO')
    ...
    it('updates channelPresetId on the node', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(0, 0, 'Node')
      store.addNode(node)

      store.applyChannelPreset(node.id, 'VERY_LONG_SLOW')

      expect(store.nodes[0].channelPresetId).toBe('VERY_LONG_SLOW')
    })

    it('sets correct sensitivity for LONG_SLOW (-129 dBm)', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(0, 0, 'Node')
      store.addNode(node)

      store.applyChannelPreset(node.id, 'LONG_SLOW')
  ...
  describe('selectedNode getter', () => {
    it('returns the node matching selectedNodeId', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(-23.55, -46.63, 'Node A')
      store.addNode(node)
    ...
    it('returns undefined after the selected node is removed', () => {
      const store = useNodesStore()
      const node = store.createDefaultNode(-23.55, -46.63, 'Node A')
      store.addNode(node)
      store.removeNode(node.id)
  ...
  describe('nodeById getter', () => {
    it('returns the correct node for the given id', () => {
      const store = useNodesStore()
      const node1 = store.createDefaultNode(-23.55, -46.63, 'Node A')
      const node2 = store.createDefaultNode(-23.56, -46.64, 'Node B')
      store.addNode(node1)
